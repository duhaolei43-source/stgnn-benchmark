...
def _evaluate(
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            if steps is not None and y_true_stats is not None and y_pred_stats is not None and err_stats is not None:
...
def _update_diag_stats(stats: Dict[str, float], values: torch.Tensor, mape_min: Optional[float]) -> None:
